...
当前 skill 假设项目目录已经存在 `.codegraph/`。一次 `codegraph_explore` 可以返回相关符号的源码、调用关系和依赖者统计；索引结果仍可能受索引滞后、解析能力和动态行为影响。

**实测已知局限（使用时需接受）：**
- 关键词匹配可能混入弱相关符号，结果通常需要 2~3 次查询逐步收窄（加入更精确的符号名/文件路径）
- 压缩/打包后的静态文件也会被索引，其"动态边界"提示多为无用噪音，可直接忽略
...
2. **调用路径/动态分发线索** — 符号之间的调用关系，包括 computed member call（配置驱动的注册/分发）的候选符号列表
3. **依赖者统计** — 调用者数量与所在文件、测试覆盖标注（⚠️ 表示无测试覆盖）。注意：这是依赖者计数列表，不是逐条的影响路径分析
4. **动态边界提示** — 静态调用链在运行时分发处终止的位置；要追踪其后的链路，对候选符号再发起一次查询即可
